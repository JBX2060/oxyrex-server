...
async function main() {
    ...
    await awaitCommand('git config credential.helper store && git config --global user.email "j.huang35@share.epsb.ca" && git config --global user.name "Jiankun-Huang" && git config --global core.excludesFile ~/.gitignore && git add . && git commit -m "Push Changes from discord" && git remote set-url origin "https://Jiankun-Huang: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@github.com/JH-Jack/oxyrex-server" && git push origin --all')
